"""Tests for releasing a claimed chore (issue #6)."""
...
def test_claimer_releases_successfully(two_households):
    client = Client()
    member = two_households["member_a"]
    chore = two_households["open_chore_a"]
    _claim_chore(chore, member)
    _session_member(client, member)

    response = client.post(f"/chores/{chore.pk}/release/")
    ...
    assert data["claimer"] is None
    assert data["claimed_at"] is None


@pytest.mark.django_db
def test_other_member_cannot_release(two_households):
    client = Client()
...
@pytest.mark.django_db
def test_admin_not_claimer_cannot_release(two_households):
    client = Client()
    claimer = two_households["member_a"]
...
@pytest.mark.django_db
def test_release_open_chore_rejected(two_households):
    client = Client()
    member = two_households["member_a"]
    chore = two_households["open_chore_a"]
    _session_member(client, member)
...
@pytest.mark.django_db
def test_release_done_chore_rejected(two_households):
    client = Client()
    member = two_households["member_a"]
    chore = two_households["open_chore_a"]
    claimed_at = timezone.now()
...
@pytest.mark.django_db
def test_cross_household_release_forbidden(two_households):
    client = Client()
    ...
    response = client.post(f"/chores/{chore_b.pk}/release/")
...
@pytest.mark.django_db
def test_unauthenticated_release_denied(two_households):
    client = Client()
    claimer = two_households["member_a"]
    chore = two_households["open_chore_a"]
    claimed_at = _claim_chore(chore, claimer)
